...
class HSplit(Widget):
    def __init__(self, items: List[Widget] = None, ratios: List[float] = None, sep=DEFAULT_SEP, item_size_mode='fixed',
                 item_align='c'):
        super().__init__()
        self.items = items or []
        for item in self.items:
...
class Grid(Widget):
    def __init__(self, items: List[Widget] = None, row_count=None, col_count=None, item_size_mode='fixed',
                 item_align='c', hsep=DEFAULT_SEP, vsep=DEFAULT_SEP, vertical=False):
        super().__init__()
        self.items = items or []
        for item in self.items:
            item.set_parent(self)
...
class TextBox(Widget):
    def __init__(self, text: str = '', style: TextStyle = None, line_count=None, line_sep=2, wrap=True,
                 overflow='shrink', use_real_line_count=False):
        """
...
class Canvas(Frame):
    ...
    async def get_img(self, scale: float = None, cache_key: str = None):
        ...
        size_limit = gallery_config.canvas_limit_size
